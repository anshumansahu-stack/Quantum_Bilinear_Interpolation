...
def build_oracle_from_neqr(neqr_qc, shape, color_depth):
    ...
    for (r_val, c_val), g_val in lookup_table.items():
        ...
        ## Google Drive
        count += 1
        # ADD THIS: Print every 50 pixels to keep the connection alive
        if count % 50 == 0:
            print(f"Oracle Build Progress: {count}/{total_pixels} pixels mapped...")
        ## End of Google Drive
