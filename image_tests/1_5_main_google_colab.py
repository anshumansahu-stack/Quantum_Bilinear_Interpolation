...
image=IP.TIF_to_numpy("Lenna_input_24.tif")
...
# --- RESUME/CHECKPOINT LOGIC --- PURELY FOR GOOGLE DRIVE
CHECKPOINT_PATH = "/content/drive/MyDrive/quantum_lena_checkpoint.npy"
PROGRESS_PATH = "/content/drive/MyDrive/last_pixel.txt"

if os.path.exists(CHECKPOINT_PATH) and os.path.exists(PROGRESS_PATH):
    result_matrix = np.load(CHECKPOINT_PATH)
    with open(PROGRESS_PATH, "r") as f:
        start_pixel = int(f.read())
    print(f"RESUMING FROM PIXEL: {start_pixel}")
else:
    result_matrix = np.zeros(new_shape[0] * new_shape[1])
    start_pixel = 0
    print("STARTING FRESH RUN...")
# --- END OF RESUME/CHECKPOINT LOGIC --- 
...
def build_circuit(original_shape, new_shape, gray_value, oracle_gate):
    ...
    for i in range(iterations):  ## Just for testing purposes, only one iteration
        ...
        # SAVE PROGRESS TO DRIVE -- GOOGLE DRIVE RELATED--
        np.save(CHECKPOINT_PATH, result_matrix)
        with open(PROGRESS_PATH, "w") as f:
            f.write(str(i + 1))
        
        if i % 10 == 0:
            print(f"Pixel {i} complete. Progress saved to Google Drive.")
        # END OF SAVE PROGRESS TO DRIVE --
