...
def build_circuit(original_shape, new_shape, gray_value, oracle_gate):
    ...
    total_qubits = (
        (5 * max_width**2) + (16 * max_width) + 4 * n + 2 * m
    )  # max_width extra for initialising value 2^2m for division at last, then required-4*n+n
    reset_per_calculation = (5 * max_width**2) + (4 * max_width) + 2 * n
    ...
    for i in range(25,26):  ## Just for testing purposes, only one iteration
        ...
        for j in range(4):  ## Change to 4 after test
            ...
            Multiplier_Ancilla_value = ((5 * max_width - 3) * max_width) // 2
            old_size = len(curr_tuple)
...
result_2d = result_matrix.reshape((4, 4), order="C")  ## Row major format
print(result_2d)
